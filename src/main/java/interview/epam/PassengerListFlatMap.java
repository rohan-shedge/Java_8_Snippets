...
public class PassengerListFlatMap {
    ...
    public static void main(String[] args) {
        ...
        Map<Character, Long> result = generateEmail(passengerList);
        System.out.println(result);
        System.out.println("--------------------------------");
        List<String> address = passengerList.stream().flatMap(p -> p.getListOfEmails().stream())
                .collect(Collectors.toList());
        System.out.println("lll " + address);
    ...
    private static Map<Character, Long> generateEmail(List<Passengers> passengerList) {
        return passengerList.stream()
                .flatMap(p -> p.getListOfEmails().stream()).map(s -> {
                        return s.charAt(2);
